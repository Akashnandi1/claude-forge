---
description: Run an independent Codex requirements review, then triage and remediate accepted findings in Claude Code.
...
<codex-review-report>
!`__CODEX_FEATURE_REVIEW__ requirement "$0"`
</codex-review-report>
...
2. If the report is `PASS`, do not invent additional review findings. State that the independent Codex gate passed and tell me the next workflow command: `/design $0`.
...
7. After approval, make only changes allowed at this gate: `claude-features/$0/requirement.md`, plus `brainstorm.md` only when the accepted correction genuinely requires a clarification there. Do not make design decisions or write code.
8. Run the relevant tests/checks after code changes when this gate permits code changes.
9. After remediation, tell me to run this same `/codex-review-requirement` command again. The gate closes only when a fresh Codex run returns PASS.

If a change contradicts a core brainstorm decision, flag that the brainstorm gate must be revisited.
