---
description: Run an independent Codex implementation-plan review, then triage and remediate accepted findings in Claude Code.
...
<codex-review-report>
!`__CODEX_FEATURE_REVIEW__ plan "$0"`
</codex-review-report>
...
2. If the report is `PASS`, do not invent additional review findings. State that the independent Codex gate passed and tell me the next workflow command: `/execute-plan 1 $0`.
...
7. After approval, make only changes allowed at this gate: `claude-features/$0/plan.md` and its phase files, plus prior docs only when a genuine upstream defect is exposed. Do not implement source code.
8. Run the relevant tests/checks after code changes when this gate permits code changes.
9. After remediation, tell me to run this same `/codex-review-plan` command again. The gate closes only when a fresh Codex run returns PASS.

If the plan is fundamentally broken, recommend re-running `/plan $0` rather than trying to preserve invalid phase boundaries.
