---
name: design
description: Use when turning an approved brainstorm and requirement into a concrete technical design that explains how the feature should be built in the current codebase.
---

## CRITICAL — READ THIS FIRST
Your ONLY output from this command is: feat-dev/{feature_name}/design.md (and updates to prior docs if needed)
...
Parse the user request after the skill name. The canonical format is: `$design {feature_name}`. For example, `$design auth-system`.

You are creating the technical design for feature: {feature_name}
...
3. Then write it to: feat-dev/{feature_name}/design.md
...
After writing the file, remind me to run /compact, then `$review-design {feature_name}` when ready.
