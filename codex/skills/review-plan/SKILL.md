---
name: review-plan
description: Use when reviewing plan.md and phase files to ensure all requirements and design work are fully covered by well-ordered, self-contained, testable phases.
---

## CRITICAL — READ THIS FIRST
Your ONLY job is to review and update plan.md and phase files in feat-dev/{feature_name}/ (and prior docs if needed)
...
Parse the user request after the skill name. The canonical format is: `$review-plan {feature_name}`. For example, `$review-plan auth-system`.

You are reviewing the implementation plan for feature: {feature_name}
...
- All files in feat-dev/{feature_name}/phases/
...
If during review you find that the plan is fundamentally broken — not just bad ordering but a misunderstanding of the design or impossible phase boundaries — tell me directly: "I think we need to re-run `$plan {feature_name}` because [reason]." Don't try to patch a broken foundation.
...
After completing the review, remind me to run /compact, then `$execute-phase 1 {feature_name}` when ready.
