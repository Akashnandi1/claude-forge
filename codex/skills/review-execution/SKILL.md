...
description: Use when reviewing the implementation of one completed phase for correctness, completeness, test coverage, and integration with prior phases before moving on.
---
...
Parse the user request after the skill name. The canonical format is: `$review-execution {phase_number} {feature_name}`. For example, `$review-execution 3 auth-system` means phase 3 of feature auth-system.
...
- All prior phase files: feat-dev/{feature_name}/phases/phase_1.md through phase_{phase_number - 1}.md (if any exist)
...
If you find that the phase implementation is fundamentally broken — not just minor issues but core logic is wrong, or the approach contradicts the design — tell me directly: "I think we need to re-run `$execute-phase {phase_number} {feature_name}` because [reason]." Don't try to patch broken code with band-aids.
...
1. **feat-dev/{feature_name}/plan.md** — Phase {phase_number} should show [x] not [ ]
2. **feat-dev/{feature_name}/phases/phase_{phase_number}.md** — completed items should show [x] not [ ]
...
- `$execute-phase {next_phase_number} {feature_name}` to continue to the next phase (where next_phase_number = phase_number + 1)
