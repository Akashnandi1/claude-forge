...
description: Use when reviewing requirement.md against brainstorm.md to ensure the specification is complete, precise, testable, and ready for technical design.
---

## CRITICAL — READ THIS FIRST
Your ONLY job is to review and update feat-dev/{feature_name}/requirement.md (and brainstorm.md if needed)
You do NOT write any code. You do NOT create any new files. You do NOT start on design, planning, or implementation.

---

Parse the user request after the skill name. The canonical format is: `$review-requirement {feature_name}`. For example, `$review-requirement auth-system`.

You are reviewing the requirements for feature: {feature_name}
...
If during review you find that the requirements are fundamentally broken — not just missing sections but based on a misunderstanding of the brainstorm or missing core functionality — tell me directly: "I think we need to re-run `$requirement {feature_name}` because [reason]." Don't try to patch a broken foundation.
...
3. Then update feat-dev/{feature_name}/requirement.md
...
After completing the review, remind me to run /compact, then `$design {feature_name}` when ready.
