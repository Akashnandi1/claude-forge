---
name: review-design
description: Use when reviewing design.md against requirement.md to ensure the technical design is complete, consistent, and implementation-ready before planning.
---

## CRITICAL — READ THIS FIRST
Your ONLY job is to review and update feat-dev/{feature_name}/design.md (and prior docs if needed)
...
Parse the user request after the skill name. The canonical format is: `$review-design {feature_name}`. For example, `$review-design auth-system`.

You are reviewing the technical design for feature: {feature_name}
...
- For changes that affect a functional requirement or acceptance criterion: flag it clearly and tell me "This change is significant enough that we should consider re-running `$review-requirement {feature_name}` after this step"
- For changes that contradict a core brainstorm decision: flag it and tell me "This change affects the brainstorm foundation — we should consider re-running `$review-brainstorm {feature_name}`"

## When to recommend re-running a prior step

If during review you find that the design is fundamentally broken — not just missing details but based on an unworkable architecture or misunderstanding of requirements — tell me directly: "I think we need to re-run `$design {feature_name}` because [reason]." Don't try to patch a broken foundation.
...
3. Then update feat-dev/{feature_name}/design.md
...
After completing the review, remind me to run /compact, then `$plan {feature_name}` when ready.
