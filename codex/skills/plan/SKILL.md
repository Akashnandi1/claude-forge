---
name: plan
description: Use when breaking an approved design into ordered, self-contained, independently testable implementation phases and writing plan.md plus phase files.
---

## CRITICAL — READ THIS FIRST
Your ONLY output from this command is:
- feat-dev/{feature_name}/plan.md
- feat-dev/{feature_name}/phases/phase_N.md files
(and updates to prior docs if needed)

You do NOT write any implementation code. You do NOT execute any phase. You do NOT create any code files. You do NOT modify any source code. You ONLY create markdown planning documents in feat-dev/. If you find yourself writing code that isn't a markdown file in feat-dev/, STOP — you have gone too far.

---

Parse the user request after the skill name. The canonical format is: `$plan {feature_name}`. For example, `$plan auth-system`.

You are creating the implementation plan for feature: {feature_name}
...
2. **Individual phase files** — One file per phase at feat-dev/{feature_name}/phases/phase_N.md, each self-contained with everything an engineer needs to implement that phase
...
- For changes that affect the architecture or a core design decision: flag it clearly and tell me "This change is significant enough that we should consider re-running `$review-design {feature_name}` after this step"
- For changes that affect requirements: flag it and tell me "This affects requirements — we should consider re-running `$review-requirement {feature_name}`"
...
3. Then create:
   - feat-dev/{feature_name}/plan.md
   - feat-dev/{feature_name}/phases/phase_1.md
   - feat-dev/{feature_name}/phases/phase_2.md
   - ... (one per phase)
...
- Do NOT skip ahead to implementation — that is the job of $execute-phase
- Do NOT read or parse conversation log files, JSONL files, or any internal Codex data files

After writing the files, remind me to run /compact, then `$review-plan {feature_name}` when ready.
